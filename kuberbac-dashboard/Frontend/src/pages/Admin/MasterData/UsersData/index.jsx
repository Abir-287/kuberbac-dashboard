...
const DataPegawai = () => {
    ...
    const filteredDataPegawai = dataPegawai.filter((pegawai) => {
        const { nama_pegawai, groups } = pegawai;
        const keyword = searchKeyword.toLowerCase();
        const groupsKeyword = filterStatus.toLowerCase();
        return (
            nama_pegawai.toLowerCase().includes(keyword) &&
            (filterStatus === '' || (groups && groups.toLowerCase().includes(groupsKeyword)))
        );
    });
    ...
    return (
        <Layout>
            ...
            <Link to="/users-data/form-data-pegawai/add">
                <ButtonOne>
                    <span>Add User</span>
                    <span>
                        <FaPlus />
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                ...
                                <th className="py-4 px-4 font-medium text-black dark:text-white xl:pl-11">Full Name</th>
                                ...
                                <th className="py-4 px-4 font-medium text-black dark:text-white">Access Right</th>
                            ...
                            {filteredDataPegawai.slice(startIndex, endIndex).map((data, index) => {
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <p className="text-black dark:text-white">{data.groups || '-'}</p>
                                        </td>
                                        <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${data.hak_akses === 'admin' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Showing {startIndex + 1}-{Math.min(endIndex, filteredDataPegawai.length)} of {filteredDataPegawai.length} Users Data
                        </span>
                    </div>
