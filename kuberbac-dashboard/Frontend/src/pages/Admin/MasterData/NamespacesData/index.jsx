            ...
            if (result.isConfirmed) {
                dispatch(deleteDataJabatan(id))
                    .then(() => {
                        Swal.fire({
                            title: 'Success',
                            text: 'Namespace data successfully deleted.',
                            icon: 'success',
                            timer: 1000,
                    ...
                    .catch((error) => {
                        Swal.fire({
                            icon: 'error',
                            title: 'Failed',
                            text: error.response?.data?.msg || error.message || 'Failed to delete namespace',
                            ...
                            {filteredDataJabatan.slice(startIndex, endIndex).map((data, index) => {
                                return (
                                    <tr key={data.id}>
                                        ...
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${data.gaji_pokok === 'Active' ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
                                                {data.gaji_pokok}
                                            </span>
                                        </td>
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                            <p className='text-black dark:text-white'>{new Date(data.tj_transport).toLocaleDateString()}</p>
                                        </td>
                                        <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                ...
                <div className="flex justify-between items-center mt-4 flex-col md:flex-row md:justify-between">
                    <div className="flex items-center space-x-2">
                        <span className="text-gray-5 dark:text-gray-4 text-sm py-4">
                            Showing {startIndex + 1}-{Math.min(endIndex, filteredDataJabatan.length)} of {filteredDataJabatan.length} Namespaces Data
                        </span>
                    </div>
